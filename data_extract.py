"""Module that has functions to return battery data from Materials Project"""
...
def get_bat_dat():
    ...
    for batt_id in all_bat_ids_list:
        ...
        #this block of code goes into the adj_pairs element of the dataframe, makes
        #it a list, and extracts something from that list
        ...
        #here we can do the same as the above line with any element in
        #adj pairs and append it to our dataframe
    ...
    all_battery_dataframe.rename(columns={'battid':'Battery ID',\
     'reduced_cell_formula':'Reduced Cell Formula', 'average_voltage':'Average Voltage (V)',\
     'min_voltage':'Min Voltage (V)', 'max_voltage':'Max Voltage (V)', 'nsteps':'Number of Steps',\
     'min_instability':'Min Instability', 'capacity_grav':'Gravimetric Capacity (units)',\
     'capacity_vol':'Volumetric Capacity', 'working_ion':'Working Ion', 'min_frac':'Min Fraction',\
     'max_frac':'Max Fraction', 'reduced_cell_composition':'Reduced Cell Composition',\
     'framework':'Framework', 'adj_pairs':'Adjacent Pairs',\
     'spacegroup':'Spacegroup', 'energy_grav':'Gravemetric Energy (units)',\
     'energy_vol':'Volumetric Energy', 'numsites':'Number of Sites', 'type':'Type'}, inplace=True)
